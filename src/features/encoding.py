...
class DeptTargetEncoder:
    ...
    def fit(self, x_train, y_train):
        ...
        self.dept_target_mean = (
            df_join_x_and_y.groupby("Dept")["Weekly_Sales"].mean().to_dict()
        )
        return self
...
class StoreTargetEncoder:
    ...
    def fit(self, x_train, y_train):
        df_join_x_and_y = pd.concat([x_train, y_train], axis=1)
        self.store_target_mean = (
            df_join_x_and_y.groupby("Store")["Weekly_Sales"].mean().to_dict()
        )
        return self
